...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#simpan").click(function() {
            var id = $('#id_pindah_masuk').val();
            var tgl_pindah_masuk = $('#tgl_pindah_masuk').val();
            var string = $('#my-form').serialize();

            if (id_pindah_masuk.length == 0) {
                alert('Maaf, ID tidak boleh kosong');
                $('id_pindah_masuk').focus();
                return false;
            }

            if (tgl_pindah_masuk.length == 0) {
                alert('Maaf, tanggal tidak boleh kosong');
                $('tgl_pindah_masuk').focus();
                return false;
            }

            $.ajax({
                type: 'POST',
                url: '<?php echo site_url(); ?>pengelola/pindah_masuk/simpan',
                data: string,
                cache: false,
    ...
    function editData(id) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?php echo site_url(); ?>pengelola/pindah_masuk/cari/' + id,
            dataType: "json",
            success: function(data) {

                $('#id_pindah_masuk').val(data.id_pindah_masuk);

                $('#tgl_pindah_masuk').val(data.tgl_pindah_masuk);
                ...
                $('$id_pindah_masuk').attr('readonly', 'true');
            }
        });
        ...
        <div class="col-md-9">
            <section class="panel panel-success">
                <header class="panel-heading">
                    ...
                    <h2 class="panel-title ">Data pindah_masuk</h2>
                </header>
                        ...
                        <table class="table table-bordered table-striped mb-none dataTable no-footer" id="datatable-editable" role="grid" aria-describedby="datatable-editable_info">
                            ...
                            <tbody>
                                ...
                                foreach ($data as $dt) {
                                    ...
                                    ?>
                                    <tr>
                                        <td class="center span1"><?php echo $dt->id_pindah_masuk; ?></td>

                                        <td class="center"><?php echo $dt->alamat_jalan; ?></td>
                                        <td class="center span2"><?php echo $dt->tgl_pindah_masuk; ?></td>

                                        <td class="center"><?php echo $dt->id_penduduk; ?></td>
                                        <td class="td-actions">
                                            <center>
                                                <div class="btn-group action-buttons">
                                                    <a href="#modal-table" onclick="javascript: editData('<?php echo $dt->id_pindah_masuk; ?>')" data-toggle="modal" title="Edit" class="btn btn-xs btn-warning"><i class="fa fa-edit"></i></a>
                                                    <a href="<?php echo site_url(); ?>pengelola/pindah_masuk/hapus/<?php echo $dt->id_pindah_masuk; ?>" onClick="return confirm('Anda yakin ingin menghapus data ini?')" data-toggle="tooltip" title="Delete" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
                                                </div>
                                            </center>
                                        </td>
                                    </tr>
                        ...
                        <fieldset>
                            ...
                            <div class="form-group control-group">
                                ...
                                <div class="col-md-8 controls">
                                    <input type="text" id="id_pindah_masuk" name="id_pindah_masuk" class="form-control" placeholder="ID" maxlength="10">
                                </div>
                            </div>
                            <div class="form-group control-group">
                                <label class="col-md-4 control-label" for="form-field-1">Nomor KK</label>
                                <div class="col-md-8 controls">
                                    <input type="text" id="tgl_pindah_masuk" name="tgl_pindah_masuk" class="form-control" placeholder="Nomor KK">
                                </div>
                            </div>
                            <div class="form-group control-group">
                                <label class="col-md-4 control-label" for="form-field-1">No Kepala Keluarga</label>
                                <div class="col-md-8 controls">
                                    <input type="text" id="nama_bayi" name="nama_bayi" class="form-control" placeholder="tgl_pindah_masukl_keluarga" maxlength="20">
                                </div>
                            </div>
                            <div class="form-group control-group">
